...
def main():
    ...
    while rospy.get_param("running"):
        ...
        #post_message("Beginning cycle " + str(curr_count))
    ...
    #post_message("Effort test has finished succesfully!")
    """now = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    msg = {"SENDER": "Youbot", "TITLE": "Test has finished", "DATE": now, "MSG": "Effort test has finished succesfully!"}
    str_msg = json.dumps(msg) + "\n"
    info = urllib.urlencode({"info": str_msg})
    urllib2.urlopen("http://danisagan-cc.netau.net/loginfo.php?" + info)"""
